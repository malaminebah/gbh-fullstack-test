...
export class VehiclesService {
  ...
  findAll(filters?: FilterOptions, pagination?: PaginationOptions): PaginatedResponse {
    ...
    if (filters) {
      if (filters.type) {
        filteredVehicles = filteredVehicles.filter(v => v.type === filters.type as VehicleType);
      }
      ...
      if (filters.fuelType) {
        filteredVehicles = filteredVehicles.filter(v => v.fuelType === filters.fuelType as FuelType);
      }
    }
